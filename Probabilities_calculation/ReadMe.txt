================================================================================
    БИБЛИОТЕКА MICROSOFT FOUNDATION CLASS. Обзор проекта Probabilities_calculation
===============================================================================

Это приложение Probabilities_calculation автоматически создано с помощью мастера приложений. Это приложение демонстрирует основные принципы использования классов Microsoft Foundation; оно также может использоваться в качестве основы для написания собственных приложений.

В этом файле представлена сводка содержимого всех файлов, входящих в состав приложения Probabilities_calculation.

Probabilities_calculation.vcxproj
...
Probabilities_calculation.vcxproj.filters
...
Probabilities_calculation.h
    Это основной файл заголовка приложения.
    Он содержит прочие заголовки, относящиеся к проекту (включая Resource.h), а также объявляет класс приложения CProbabilities_calculationApp.

Probabilities_calculation.cpp
    Это основной исходный файл приложения, в котором содержится класс приложения CProbabilities_calculationApp.

Probabilities_calculation.rc
...
res\Probabilities_calculation.ico
    Файл значка, используемый в качестве значка приложения. Этот значок включен с помощью основного файла ресурсов Probabilities_calculation.rc.

res\Probabilities_calculation.rc2
...
Probabilities_calculationDlg.h, Probabilities_calculationDlg.cpp — диалоговое окно
    Эти файлы содержат класс CProbabilities_calculationDlg. Этот класс определяет поведение главного диалогового окна приложения. Шаблон диалогового окна находится в файле Probabilities_calculation.rc, который можно изменять с помощью Microsoft Visual C++.
...
StdAfx.h, StdAfx.cpp
    Эти файлы используются для построения файла предкомпилированного заголовка (PCH) с именем Probabilities_calculation.pch и файла предкомпилированных типов с именем StdAfx.obj.
...
Probabilities_calculation.manifest
